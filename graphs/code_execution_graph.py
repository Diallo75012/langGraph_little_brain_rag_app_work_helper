...
def documentation_writer(state: MessagesState, apis = apis, doc_writer_class = DocumentationWriter):
  ...
  try:
    ...
    schema={
      "documentation": "documentation and guidance for Python script creation about what the user needs. Just ouput the documentation with all steps for Python developer to understand how to write the script. use one unique block valid JSON str to answer. Ensure all special characters (like backslashes, quotes, and newlines) are properly escaped.\n- The documentation must be valid JSON and should be easily parsed into a Python dictionary without errors."
    }

    '''

    # original model having error sometimes with format output: groq_llm_llama3_70b
 ...
 # parse content, here doc is dict
    ...
    apis_links = os.getenv("API_CHOICE")
    system_prompt = documentation_writer_prompt["system"]["template"]
    human_prompt = documentation_writer_prompt["human"]["template"]
    system_message = eval(f'f"""{system_prompt}"""')
    print("System prompt doc creation: ", system_message)
    human_message = eval(f'f"""{human_prompt}"""')
    print("Human prompt doc creation: ", human_message)
    messages = [system_message, human_message]
    response = groq_llm_llama3_70b.invoke(messages)
    print("Raw Documentation Response: ", response)
    doc = response.content.split("```")[1].strip("markdown").strip()
    print("Documentation Parsed: ", doc)
...
def llama_3_8b_script_creator(state: MessagesState, apis=apis, script_creation_class = ScriptCreation, llm = groq_llm_llama3_8b):
  ...
  # agent choosen api to satisfy user api call code generation
  api_choice = os.getenv("API_CHOICE")
  # user initial query or user initial query and error message if we are in a code re-creation loop from another node judging code
  if "error" in last_message:
    user_initial_query = os.getenv("USER_INITIAL_QUERY") + f". We already executed the code before and got an error: {last_message} .Make sure that the new script don't have any errors."
  else:
    user_initial_query = os.getenv("USER_INITIAL_QUERY")
...
def choose_code_to_execute_node_if_many(state: MessagesState, comparator_choice_class = CodeComparison):
  ...
  try:
    ...
    schema={
      "name": "Codes are labelled with names. Answer the name of the code that you have selected as being the best out the choices. If all codes are the same, just choose one of the available name as value. You MUST choose one of the available names choices. use str to answer.",
    ...
    for agent_script_file_name in os.listdir(agents_scripts_dir):
      if agent_script_file_name.startswith(f"agent_code_execute_in_docker_{llm_name}"):
        ...
        try:
          ...
          exception_error_log_list_dict.append({
            "script_file_path": f"{agents_scripts_dir}/{agent_script_file_name}",
            "llm_script_origin": llm_name,
...
def rewrite_documentation_or_execute(state: MessagesState):
  ...
  messages = state["messages"]
  last_message = messages[-1].content
  print("Last message before JSON parsing in conditional edge 'rewrite_documentation_or_execute':", last_message)
...
def rewrite_or_create_requirements_decision(state: MessagesState):
  ...
  if "error" in evaluations or retry_code_validation == 0:
    # if there is no more reties we will just fail and go to error_handler
    if retry_code_validation == 0:
      ...
      return "error_handler"
    # if there is an error from previous step but we still have some retries left we send the error and redo the parallele execution
    elif "error" in evaluations and retry_code_validation < 0:
  ...
  ### this triggers if there is no error notification from previous step ###
  ...
  # if we have a least one valid code we can try execution route with last code check before execution in docker
...
def success_execution_or_retry_or_return_error(state: MessagesState):
  ...
  # if no code executed well we need to analyze the error or send those errors to a certain node for retry
    ...
    if os.getenv("RETRY_CODE_EXECUTION") == 0:
      # if the execution reties are equal to zero we return the execption error
      code_execution_result["exception"] = code_execution_result["stderr"] + code_execution_result["exception"]
      print("Code execution result Exception Triggered and No More Retries: ", code_execution_result["exception"])
    ...
    elif code_execution_result["exception"] and os.getenv("RETRY_CODE_EXECUTION") < 0:
      # add to the stderr the exception message if any we add it to the stderr message so that llm have full error message to debug
      code_execution_result["stderr"] = code_execution_result["stderr"] + code_execution_result["exception"]
      
      print("Code execution result Exception Triggered: ", code_execution_result["stderr"], "code executions left:", os.getenv("RETRY_CODE_EXECUTION"))
      # otherwise we check if there is an exception and will return the result error_handler node that will handle the message
